...
const BLITZ_TIME_SECONDS = 30;
const CHALLENGE_START_IST = new Date("2026-04-11T00:00:00+05:30").getTime();
const DAY_SIX_START_IST = new Date("2026-04-16T00:00:00+05:30").getTime();
...
const quizQuestions: QuizQuestion[] = [
  {
    id: 1,
    difficulty: "Easy",
    question: "Which quantity has SI unit Newton?",
    options: ["Pressure", "Force", "Work", "Power"],
    answer: "Force",
  },
  {
    id: 2,
    difficulty: "Easy",
    question: "Molar mass of water (H2O) is approximately:",
  ...
  },
  {
    id: 4,
    difficulty: "Medium",
    question: "Power factor of a purely resistive AC circuit is:",
    options: ["0", "0.5", "1", "-1"],
    answer: "1",
  },
  {
  ...
  },
  {
    id: 7,
    difficulty: "Hard",
    question: "For ideal gas process, PV = nRT indicates that at constant T, P is:",
    options: ["Directly proportional to V", "Inversely proportional to V", "Independent of V", "Equal to V"],
    answer: "Inversely proportional to V",
  },
  {
    id: 8,
    difficulty: "Hard",
    question: "Eigenvalues of a triangular matrix are its:",
    options: ["First row values", "Determinant", "Diagonal entries", "Trace only"],
    answer: "Diagonal entries",
  },
  {
    id: 9,
    difficulty: "Hard",
    question: "The rate constant k for first-order reactions has units:",
    options: ["s^-1", "mol L^-1 s^-1", "L mol^-1 s^-1", "mol^-1"],
    answer: "s^-1",
  },
  {
    id: 10,
    difficulty: "Expert",
    question: "If y = e^(2x), then d2y/dx2 equals:",
    options: ["2e^(2x)", "4e^(2x)", "e^(2x)", "8e^(2x)"],
    answer: "4e^(2x)",
  },
];

const blitzQuestions: BlitzQuestion[] = [
  { id: 1, question: "8 + 5 = ?", options: [11, 12, 13, 14], answerIndex: 2, weight: 1 },
  { id: 2, question: "14 - 9 = ?", options: [3, 4, 5, 6], answerIndex: 2, weight: 2 },
  { id: 3, question: "6 x 7 = ?", options: [36, 40, 42, 48], answerIndex: 2, weight: 3 },
  { id: 4, question: "45 / 9 = ?", options: [4, 5, 6, 7], answerIndex: 1, weight: 4 },
  { id: 5, question: "3^3 + 4 = ?", options: [27, 29, 31, 33], answerIndex: 2, weight: 5 },
  { id: 6, question: "If 2x + 5 = 17, x = ?", options: [5, 6, 7, 8], answerIndex: 1, weight: 6 },
  { id: 7, question: "12% of 250 = ?", options: [25, 30, 35, 40], answerIndex: 1, weight: 7 },
  { id: 8, question: "sqrt(625) = ?", options: [20, 22, 24, 25], answerIndex: 3, weight: 8 },
  { id: 9, question: "LCM of 12 and 18 = ?", options: [24, 30, 36, 42], answerIndex: 2, weight: 9 },
  { id: 10, question: "If f(x)=x^2-3x, then f(6)= ?", options: [18, 20, 24, 30], answerIndex: 0, weight: 10 },
];

const QUIZ_POINT_PER_CORRECT = 5;
const BLITZ_MAX_UNITS = blitzQuestions.reduce((sum, item) => sum + item.weight, 0);

export default function DaySixPage() {
  const router = useRouter();
  const [user, setUser] = useState<User | null>(null);
  const [ready, setReady] = useState(false);
  const [stage, setStage] = useState<"quiz" | "blitz">("quiz");
  ...
  const [blitzTimeLeft, setBlitzTimeLeft] = useState(BLITZ_TIME_SECONDS);
  const [blitzIndex, setBlitzIndex] = useState(0);
  const [blitzAnswers, setBlitzAnswers] = useState<Array<number | null>>(
    Array(blitzQuestions.length).fill(null)
  );
  ...
  useEffect(() => {
    if (!blitzStarted || blitzCompleted) return;
    if (blitzTimeLeft <= 0) {
    ...
    const intervalId = window.setInterval(() => {
      setBlitzTimeLeft(prev => Math.max(prev - 1, 0));
    }, 1000);

    return () => window.clearInterval(intervalId);
  }, [blitzCompleted, blitzStarted, blitzTimeLeft]);
  ...
  const blitzCorrectCount = useMemo(
    () =>
      blitzQuestions.reduce((sum, item, index) => {
        return sum + (blitzAnswers[index] === item.answerIndex ? 1 : 0);
      }, 0),
    [blitzAnswers]
  );

  const blitzUnits = useMemo(
    () =>
      blitzQuestions.reduce((sum, item, index) => {
        return sum + (blitzAnswers[index] === item.answerIndex ? item.weight : 0);
      }, 0),
    [blitzAnswers]
  );
  ...
  const handleSubmit = useCallback(
    async (autoSubmitted: boolean) => {
      ...
      if (!autoSubmitted && !blitzCompleted) {
        alert("Please complete Math Blitz.");
        return;
      }
      ...
      try {
        const dayRef = doc(db, "users", user.uid, "days", "day6");
        const existingDaySubmission = await getDoc(dayRef);
        if (existingDaySubmission.exists()) {
          alert("You have already submitted Day 6. Only one submission is allowed.");
          setSubmitted(true);
        ...
        const blitzPoints = blitzUnits;
        const basePoints = quizPoints + blitzPoints;

        const totalCorrectCount = quizCorrect + blitzUnits;
        const totalQuestionCount = quizQuestions.length + BLITZ_MAX_UNITS;
        ...
        const daysLate = Math.max(currentChallengeDay - 6, 0);
        ...
        batch.set(
          dayRef,
          {
            ...
            blitzUnits,
            blitzPoints,
            blitzTimeLimitSeconds: BLITZ_TIME_SECONDS,
            blitzTimeRemainingSeconds: blitzTimeLeft,
            basePoints,
            totalCorrectCount,
            totalQuestionCount,
            timeBonusPerCorrect,
        ...
        console.error("Failed to submit Day 6 challenge", error);
  ...
  if (!ready) {
    return renderPage(
      <div className="flex min-h-[70vh] items-center justify-center text-white">
        <p className="text-sm text-white/70">Loading Day 6 challenge...</p>
      </div>
    );
  }

  if (!TEST_FORCE_UNLOCK && nowMs < DAY_SIX_START_IST) {
    return renderPage(
      <div className="flex min-h-[70vh] items-center justify-center text-white">
        <section className="w-full max-w-2xl rounded-2xl border border-[#f47a20]/35 bg-[#0b0b0b] p-6 text-center">
          <h1 className="text-2xl font-semibold text-[#f47a20]">Day 6 Locked</h1>
          <p className="mt-3 text-sm text-white/70">Day 6 unlocks in</p>
          <p className="mt-1 text-lg font-semibold text-white">{formatCountdown(DAY_SIX_START_IST - nowMs)}</p>
          <button
            type="button"
  ...
  const activeBlitzQuestion = blitzQuestions[blitzIndex];
  const allQuizAnswered = quizAnswers.every(answer => answer !== "");
  const showSubmitButton = stage === "blitz" && blitzCompleted && allQuizAnswered;
  ...
  return renderPage(
    <section className="mx-auto w-full max-w-4xl space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-semibold text-[#f47a20]">Day 6 Challenge</h1>
          <p className="mt-1 text-sm text-white/70">Quiz first, then Math Blitz mini-game</p>
        </div>
        ...
        <section className="rounded-2xl border border-[#f47a20]/30 bg-[#0b0b0b] p-5">
          {!blitzStarted ? (
            <div className="rounded-2xl border border-white/15 bg-[#111111] p-6 text-center">
              <h2 className="text-3xl font-semibold text-[#f2f2f2]">Math Blitz</h2>
              <p className="mt-3 text-lg text-[#87b6ad]">Solve 10 math problems in 30 seconds!</p>
              <button
                type="button"
                onClick={() => {
                  setBlitzStarted(true);
                  setBlitzCompleted(false);
                  setBlitzTimeLeft(BLITZ_TIME_SECONDS);
                  setBlitzIndex(0);
                  setBlitzAnswers(Array(blitzQuestions.length).fill(null));
                }}
            ...
            <div className="rounded-2xl border border-white/15 bg-[#111111] p-4 sm:p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-2xl font-semibold">Math Blitz</h2>
                <div className="rounded-full border border-emerald-500/50 bg-emerald-500/10 px-4 py-1.5 text-sm font-semibold text-emerald-300">
                  {blitzCompleted ? "Completed" : `00:${String(blitzTimeLeft).padStart(2, "0")}`}
                </div>
              </div>
              <p className="mt-2 text-sm text-white/70">
                Q{Math.min(blitzIndex + 1, blitzQuestions.length)}/{blitzQuestions.length} | Correct: {blitzCorrectCount} | Weighted Score: {blitzUnits}
              </p>

              {!blitzCompleted ? (
                <>
                  <div className="mt-4 rounded-xl border border-white/10 bg-[#151515] p-4">
                    <p className="text-base font-medium">{activeBlitzQuestion.question}</p>
                    <p className="mt-1 text-xs text-white/55">
                      Difficulty Weight: {activeBlitzQuestion.weight}
                    </p>
                    <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      {activeBlitzQuestion.options.map((option, optionIndex) => (
                        <button
                          key={`${activeBlitzQuestion.id}-${option}`}
                          type="button"
                          onClick={() => {
                            setBlitzAnswers(prev => {
                              const next = [...prev];
                              next[blitzIndex] = optionIndex;
                              return next;
                            });

                            if (blitzIndex >= blitzQuestions.length - 1) {
                              setBlitzCompleted(true);
                              return;
                            }
                ...
                <div className="mt-4 rounded-xl border border-white/10 bg-[#151515] p-4 text-sm text-white/80">
                  <p>Math Blitz complete.</p>
                  <p className="mt-1">Correct Answers: {blitzCorrectCount}/{blitzQuestions.length}</p>
                  <p className="mt-1">Weighted Blitz Units: {blitzUnits}/{BLITZ_MAX_UNITS}</p>
                </div>
              )}
            </div>
          )}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {showSubmitButton ? (
          <button
            ...
            onClick={() => {
              const confirmed = window.confirm(
                "Confirm final submission for Day 6? You can submit only once."
              );
              if (!confirmed) return;
          ...
          >
            {submitting ? "Submitting..." : submitted ? "Submitted" : "Submit Day 6"}
          </button>
        ) : null}
      </div>
    </section>
